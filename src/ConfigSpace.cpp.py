...
def RandomlySampleConfigLine(self):
    wt1 = rand() / float(RAND_MAX)
    wt2 = 1.f - wt1
    pos = wt1 * m_pos1 + wt2 * m_pos2
    return pos


def RandomlySampleConfigLineDiscrete(self):
    r = rand() / float(RAND_MAX)
    pos = (r >= 0.5f) ? m_pos1 : m_pos2
    return pos


def GetConfigLineLength(self):
    return mag(m_pos1 - m_pos2)


def GetConfigLineSqLength(self):
    return mag2(m_pos1 - m_pos2)


def PrintConfigLine(self):
    std.cout << "p1: " << m_pos1 << "; p2: " << m_pos2 << std.endl


def TranslateConfigLine(self, trans):
    m_pos1 = m_pos1 + trans
    m_pos2 = m_pos2 + trans
...
CConfigSpace.CConfigSpace( CRoom& room1, room2)
    type1 = room1.GetTemplateType()
    type2 = room2.GetTemplateType()
    if m_flagPrecomputed == True and type1 >= 0 and type2 >= 0:
        cs = m_precomputedTable[type1][type2]
...
#endif
                pos1 = room2.GetRoomCenter() + pr1
                pos2 = room2.GetRoomCenter() + pr2
...
def RandomlySampleConfigSpaceContinuous(self):
    ...
    lineIndex = int(rand() / float(RAND_MAX) * numOfLines)
    lineIndex = lineIndex % numOfLines
    pos = m_vecConfigLine[lineIndex].RandomlySampleConfigLine()
    return pos
...
def RandomlySampleConfigSpaceDiscrete(self):
    ...
    lineIndex = int(rand() / float(RAND_MAX) * numOfLines)
    lineIndex = lineIndex % numOfLines
    pos = m_vecConfigLine[lineIndex].RandomlySampleConfigLineDiscrete()
    return pos
...
def SmartlySampleConfigSpace(self):
    ...
    for (i = 0; i < numOfLines; i++)
        ...
        r = rand() / float(RAND_MAX)
        pos = (r >= 0.5f) ? configLine.RandomlySampleConfigLine() : configLine.RandomlySampleConfigLineDiscrete()
        vecPos[i] = pos
...
def FindIntersection(self, configSpace1, configSpace2):
    ...
    for (i1 = 0; i1 < configSpace1.GetNumOfLines(); i1++)
        ...
        for (i2 = 0; i2 < configSpace2.GetNumOfLines(); i2++)
            ...
            if configLine1.GetConfigLineSqLength() < g_numericalToleranceSq and configLine2.GetConfigLineSqLength() < g_numericalToleranceSq:
                if mag2(configLine1.GetPos1() - configLine2.GetPos1()) < g_numericalToleranceSq:
                    intersectSpace.AddConfigLine(configLine1)

                continue

            elif configLine1.GetConfigLineSqLength() < g_numericalToleranceSq:
            ...
            elif configLine2.GetConfigLineSqLength() < g_numericalToleranceSq:
...
def TranslateConfigSpace(self, trans):
    for (i = 0; i < GetNumOfLines(); i++)
