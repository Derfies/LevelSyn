...
class LevelSynth:
        ...
        for (i = 0; i < picked_room.GetNumOfEdges(); i++):
            ...
            for (j = 0; j < self.layout.Getnum_rooms(); j++):
                ...
                for (k = 0; k < otherRoom.GetNumOfEdges(); k++):
                    otherEdge = otherRoom.get_edge(k)
    ...
    def init_scene(self):
        ...
        numOfTemplates = self.templates.num_templates
    ...
    def RandomlyPickAnotherRoom(self, layout, picked_index):
        num_rooms = layout.Getnum_rooms()
        otherRoomIndex = picked_index
        while otherRoomIndex == picked_index:
            otherRoomIndex = int(rand() / float(RAND_MAX) * num_rooms)
            otherRoomIndex = otherRoomIndex % num_rooms
        return otherRoomIndex

    def GetConnectedIndices(self, graph, picked_index, flag_visited_only=True):
        indices = []
    ...
    def sample_config_space_for_picked_room(self, layout, graph, indices, picked_room_index):
        ...
        if connectedIndices.size() >= 1:
            ...
            Cconfig_space config_space0(layout.get_room(idx0), picked_room)
            config_space = config_space0
            for (i = 1; i < int(connectedIndices.size()); i++):
                Cconfig_space config_spaceTmp(layout.get_room(connectedIndices[i]), picked_room)
                config_spaceNew = Cconfig_space.FindIntersection(config_space, config_spaceTmp)
        ...
        while (config_space.IsEmpty() == True)
            otherRoomIndex = RandomlyPickAnotherRoom(layout, picked_room_index)
            otherRoom = layout.get_room(otherRoomIndex)
            config_space = Cconfig_space(otherRoom, picked_room)
            whileCnt++
            if whileCnt >= 1000:
